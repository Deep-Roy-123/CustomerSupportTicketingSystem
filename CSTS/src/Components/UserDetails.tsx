...
const UserDetails:React.FC<UserDetailsProps> = ({showForm,setShowForm,userId,setUserId,userData, darkMode}) => {
    const navigate=useNavigate();
    const uData = userData.find((d) => d.id === userId) as User;
  return (
    <div className={`${(darkMode===true)?'bg-gray-800 text-gray-200 border-gray-200':'bg-gray-200 text-gray-800 border-gray-800'}`}>
        {showForm && (
